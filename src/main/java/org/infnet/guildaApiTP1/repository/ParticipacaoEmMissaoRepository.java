...
import org.infnet.guildaApiTP1.dto.UltimaMissaoDTO;
...
public interface ParticipacaoEmMissaoRepository extends JpaRepository<ParticipacaoEmMissao, Long> {
    ...
    @Query("""
                select new org.infnet.guildaApiTP1.dto.UltimaMissaoDTO(
                    m.id,
                    m.titulo,
                    m.nivelDePerigo,
                    m.status,
    ...
    @Query("""
                    select new org.infnet.guildaApiTP1.dto.ParticipanteDTO(
                        a.id,
                        a.nome,
                        p.papelNaMissao,
                        p.recompensaEmOuro,
    ...
    @Query("""
                    select new org.infnet.guildaApiTP1.dto.RelatorioParticipacaoDTO(
                        a.id,
                        a.nome,
                        a.nivel,
                        count(p),
    ...
    @Query("""
               select new org.infnet.guildaApiTP1.dto.RelatorioMissaoDTO(
                        m.titulo,
                        m.status,
                        m.nivelDePerigo,
                        count(p),
